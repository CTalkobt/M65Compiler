    .o45
    .org $2000
    .weak __sp_base
    __sp_base = $0101
    .weak __static_chain
    .weak __zp_scratch
    .weak __zp_scratch2
    .weak __zp_scratch3
    .weak __zp_scratch4
    .weak cc45.zeroPageStart
    __static_chain = $06
    __zp_scratch = $08
    __zp_scratch2 = $0A
    __zp_scratch3 = $0C
    __zp_scratch4 = $0E
    cc45.zeroPageStart = $08

    .global _results
    .global _g_value
    .global _g_flag
    .global _store_to_global
    .global _set_flag
    .global _copy_via_asm
    .global _test_local_access
    .global _main

    .segment "data"
    .byte 0
_results:
; .debug_var: @global _results offset=0 size=2 type=ptr scope=global
    .word 0
_g_value:
; .debug_var: @global _g_value offset=0 size=2 type=int16 scope=global
    .word 0
_g_flag:
; .debug_var: @global _g_flag offset=0 size=2 type=int8 scope=global
    .byte 0

    .segment "code"

; function _store_to_global
; SAC inline storage: 2 bytes
    .global _store_to_global__param_val
    _store_to_global__param_val: .word 0
    _store_to_global__local_0: .word 0
    proc _store_to_global, W#@_p_val
    .sac
    .var _fp = 0
    .loc "src/test-resources/test_inline_asm.c", 11
    .var @_p_val = 2
; .debug_var: __store_to_global @_p_val offset=2 size=2 type=int16 scope=parameter

@entry:
    .loc "src/test-resources/test_inline_asm.c", 13
    ldax @_p_val, sp
    .loc "src/test-resources/test_inline_asm.c", 14
    stax _g_value
@__return:
    rts
    .func_flags stack_call, static_alloc, leaf
    .reg_clobbers A, X, Y, Z
    .flag_clobbers C, N, Z, V
    .frame_size 2
    endproc

; function _set_flag
; SAC inline storage: 2 bytes
    .global _set_flag__param_f
    _set_flag__param_f: .word 0
    _set_flag__local_0: .word 0
    proc _set_flag, B#@_p_f
    .sac
    .var _fp = 0
    .loc "src/test-resources/test_inline_asm.c", 17
    .var @_p_f = 2
; .debug_var: __set_flag @_p_f offset=2 size=2 type=int8 scope=parameter

@entry:
    .loc "src/test-resources/test_inline_asm.c", 19
    lda @_p_f, sp
    .loc "src/test-resources/test_inline_asm.c", 20
    sta _g_flag
@__return:
    rts
    .func_flags stack_call, static_alloc, leaf
    .reg_clobbers A, X, Y, Z
    .flag_clobbers C, N, Z, V
    .frame_size 2
    endproc

; function _copy_via_asm
; SAC inline storage: 4 bytes
    .global _copy_via_asm__param_val
    _copy_via_asm__param_val: .word 0
    _copy_via_asm__local_0: .word 0
    _copy_via_asm__local_1: .word 0
    proc _copy_via_asm, W#@_p_val
    .sac
    .var _fp = 0
    .loc "src/test-resources/test_inline_asm.c", 23
    .local @_l_result = 2
; .debug_var: __copy_via_asm @_l_result offset=2 size=2 type=int16 scope=local
    .var @_p_val = 2
; .debug_var: __copy_via_asm @_p_val offset=2 size=2 type=int16 scope=parameter

@entry:
    .loc "src/test-resources/test_inline_asm.c", 24
    lda #0
    sta _copy_via_asm__local_1
    sta _copy_via_asm__local_1+1
    .loc "src/test-resources/test_inline_asm.c", 26
    ldax @_p_val, sp
    .loc "src/test-resources/test_inline_asm.c", 27
    stax @_l_result, sp
    .loc "src/test-resources/test_inline_asm.c", 28
    lda _copy_via_asm__local_1
    ldx _copy_via_asm__local_1+1
@__return:
    rts
    .func_flags stack_call, static_alloc, leaf
    .reg_clobbers A, X, Y, Z
    .flag_clobbers C, N, Z, V
    .frame_size 4
    endproc

; function _test_local_access
; SAC inline storage: 4 bytes
    _test_local_access__local_0: .word 0
    _test_local_access__local_1: .word 0
    _test_local_access__local_3: .word 0
    _test_local_access__local_4: .word 0
    proc _test_local_access
; Phase 51: zero-alloc leaf (all parameters constant)
    .var _fp = 0
    .loc "src/test-resources/test_inline_asm.c", 31
    .local @_l_d = 0
    .local @_l_x = 2
; .debug_var: __test_local_access @_l_d offset=0 size=2 type=int16 scope=local
; .debug_var: __test_local_access @_l_x offset=2 size=2 type=int16 scope=local

@entry:
    .loc "src/test-resources/test_inline_asm.c", 33
    lda #50
    ldx #0
    sta _copy_via_asm__param_val
    stx _copy_via_asm__param_val+1
    jsr _copy_via_asm
    .loc "src/test-resources/test_inline_asm.c", 36
    lda #100
    ldx #0
    sta _copy_via_asm__param_val
    stx _copy_via_asm__param_val+1
    jsr _copy_via_asm
    sta _test_local_access__local_3
    stx _test_local_access__local_3+1
    .loc "src/test-resources/test_inline_asm.c", 37
    lda _test_local_access__local_3
    ldx _test_local_access__local_3+1
    cmp.16 .AX, #100
    beq @if_then0
    bra @if_end2
@if_then0:
    .loc "src/test-resources/test_inline_asm.c", 38
    lda #1
    sta $20
    lda _results
    ldx _results+1
    sta $22
    stx $23
    lda #3
    ldx #0
    lda $20
    ldx #0
    pha
    sta __zp_scratch3
    stx __zp_scratch3+1
    lda $22
    ldx $22+1
    clc
    adc __zp_scratch3
    pha
    txa
    adc __zp_scratch3+1
    tax
    pla
    sta __zp_scratch
    stx __zp_scratch+1
    pla
    ldy #0
    sta (__zp_scratch),y
@if_end2:
@__return:
    rts
    .func_flags stack_call, static_alloc
    .reg_clobbers A, X, Y, Z
    .flag_clobbers C, N, Z, V
    .frame_size 4
    endproc

; function _main
; SAC inline storage: 4 bytes
    _main__local_1: .word 0
    _main__local_10: .word 0
    _main__local_20: .word 0
    _main__local_28: .word 0
    _main__local_29: .word 0
    _main__local_31: .word 0
    _main__local_32: .word 0
    _main__local_40: .word 0
    _main__local_41: .word 0
    _main__local_42: .word 0
    _main__local_43: .word 0
    proc _main
; Phase 51: zero-alloc leaf (all parameters constant)
    .var _fp = 0
    .loc "src/test-resources/test_inline_asm.c", 42
    .local @_l_d = 0
    .local @_l_x = 2
; .debug_var: __main @_l_d offset=0 size=2 type=int16 scope=local
; .debug_var: __main @_l_x offset=2 size=2 type=int16 scope=local

@entry:
    .loc "src/test-resources/test_inline_asm.c", 43
    lda #0
    ldx #64
    sta _results
    stx _results+1
    .loc "src/test-resources/test_inline_asm.c", 46
    lda #171
    ldx #0
    sta _store_to_global__param_val
    stx _store_to_global__param_val+1
    jsr _store_to_global
    .loc "src/test-resources/test_inline_asm.c", 47
    lda _g_value
    ldx _g_value+1
    cmp.16 .AX, #171
    beq @if_then3
    bra @if_end5
@if_then3:
    .loc "src/test-resources/test_inline_asm.c", 48
    lda #1
    sta $20
    lda _results
    ldx _results+1
    sta $22
    stx $23
    lda #0
    tax
    lda $20
    ldx #0
    pha
    sta __zp_scratch3
    stx __zp_scratch3+1
    lda $22
    ldx $22+1
    clc
    adc __zp_scratch3
    pha
    txa
    adc __zp_scratch3+1
    tax
    pla
    sta __zp_scratch
    stx __zp_scratch+1
    pla
    ldy #0
    sta (__zp_scratch),y
@if_end5:
    .loc "src/test-resources/test_inline_asm.c", 52
    lda #66
    ldx #0
    sta _main__local_10
    lda _main__local_10
    sta _set_flag__param_f
    stx _set_flag__param_f+1
    jsr _set_flag
    .loc "src/test-resources/test_inline_asm.c", 53
    lda _g_flag
    ldx #0
    lda #66
    sxt.8
    ldx #0
    sta __zp_scratch2
    stx __zp_scratch2+1
    cmp.16 .AX, __zp_scratch2
    beq @if_then6
    bra @if_end8
@if_then6:
    .loc "src/test-resources/test_inline_asm.c", 54
    lda #1
    sta $20
    lda _results
    ldx _results+1
    sta $22
    stx $23
    lda #1
    ldx #0
    lda $20
    ldx #0
    pha
    sta __zp_scratch3
    stx __zp_scratch3+1
    lda $22
    ldx $22+1
    clc
    adc __zp_scratch3
    pha
    txa
    adc __zp_scratch3+1
    tax
    pla
    sta __zp_scratch
    stx __zp_scratch+1
    pla
    ldy #0
    sta (__zp_scratch),y
@if_end8:
    .loc "src/test-resources/test_inline_asm.c", 58
    lda #0
    ldx #0
    sta _store_to_global__param_val
    stx _store_to_global__param_val+1
    jsr _store_to_global
    .loc "src/test-resources/test_inline_asm.c", 59
    lda _g_value
    ldx _g_value+1
    stx __zp_scratch
    ora __zp_scratch
    beq @if_then9
    bra @if_end11
@if_then9:
    .loc "src/test-resources/test_inline_asm.c", 60
    lda #1
    sta $20
    lda _results
    ldx _results+1
    sta $22
    stx $23
    lda #2
    ldx #0
    lda $20
    ldx #0
    pha
    sta __zp_scratch3
    stx __zp_scratch3+1
    lda $22
    ldx $22+1
    clc
    adc __zp_scratch3
    pha
    txa
    adc __zp_scratch3+1
    tax
    pla
    sta __zp_scratch
    stx __zp_scratch+1
    pla
    ldy #0
    sta (__zp_scratch),y
@if_end11:
    .loc "src/test-resources/test_inline_asm.c", 33
    lda #50
    ldx #0
    sta _copy_via_asm__param_val
    stx _copy_via_asm__param_val+1
    jsr _copy_via_asm
    .loc "src/test-resources/test_inline_asm.c", 36
    lda #100
    ldx #0
    sta _copy_via_asm__param_val
    stx _copy_via_asm__param_val+1
    jsr _copy_via_asm
    sta _main__local_31
    stx _main__local_31+1
    .loc "src/test-resources/test_inline_asm.c", 37
    lda _main__local_31
    ldx _main__local_31+1
    cmp.16 .AX, #100
    beq @if_then13
    bra @if_end15
@if_then13:
    .loc "src/test-resources/test_inline_asm.c", 38
    lda #1
    sta $20
    lda _results
    ldx _results+1
    sta $22
    stx $23
    lda #3
    ldx #0
    lda $20
    ldx #0
    pha
    sta __zp_scratch3
    stx __zp_scratch3+1
    lda $22
    ldx $22+1
    clc
    adc __zp_scratch3
    pha
    txa
    adc __zp_scratch3+1
    tax
    pla
    sta __zp_scratch
    stx __zp_scratch+1
    pla
    ldy #0
    sta (__zp_scratch),y
@if_end15:
@__return:
    rts
    .func_flags stack_call, static_alloc
    .reg_clobbers A, X, Y, Z
    .flag_clobbers C, N, Z, V
    .frame_size 4
    endproc


__zp_save_buf:
