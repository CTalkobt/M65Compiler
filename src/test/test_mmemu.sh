...
compile_link_test "src/test-resources/test_short.c" "build/test/test_short.prg"
if [ $? -ne 0 ]; then
    echo "FAIL: Compilation/linking failed for test_short.c"
    failed=$((failed + 1))
else
...
compile_link_test "src/test-resources/test_struct_return.c" "build/test/test_struct_return.prg"
if [ $? -ne 0 ]; then
    echo "FAIL: Compilation/linking failed for test_struct_return.c"
    failed=$((failed + 1))
else
