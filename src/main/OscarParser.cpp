...
AsmIR::Module OscarParser::parse(const std::string& source) {
    AsmIR::Module module;
...
std::vector<OscarParser::Token> OscarParser::tokenize(const std::string& source) {
    ...
    while (pos < source.length()) {
        ...
        // Comments (* at line start or ; anywhere)
        if ((c == '*' && column == 1) || c == ';') {
        ...
        // Numbers (hex $xxxx, decimal, can have optional leading digits)
        ...
        // Single character tokens first (to handle : properly)
...
AsmIR::Module OscarParser::parseTokens(const std::vector<Token>& tokens) {
    AsmIR::Module module;
    size_t pos = 0;
    ...
    while (peek().type != TokenType::END_OF_FILE) {
        ...
        if (tok.type == TokenType::IDENTIFIER) {
            ...
            // Check if this is a label: followed by colon, directive, mnemonic, newline, or EOF
            // (Oscar labels can appear on their own line or before a directive/instruction/mnemonic)
            bool is_label = peek().type == TokenType::COLON ||
                           peek().type == TokenType::DIRECTIVE ||
...
AsmIR::AddressingMode OscarParser::inferAddressingMode(const std::string& operand) {
...
std::string OscarParser::normalizeMnemonic(const std::string& mnemonic) {
    std::string result = mnemonic;
    std::transform(result.begin(), result.end(), result.begin(), ::toupper);
    return result;
}

bool OscarParser::isDirective(const std::string& word) {
...
AsmIR::DirectiveType OscarParser::directiveTypeFromName(const std::string& name) {
    ...
    if (lower == "hex") return AsmIR::DirectiveType::BYTE_DATA;  // HEX maps to BYTE_DATA
...
bool OscarParser::isMnemonic(const std::string& word) {
