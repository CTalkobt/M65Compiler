...
public:
    Phase107TuningHooks(Phase107OnlineLearner* learner = nullptr)
...
private:
    Phase107OnlineLearner* onlineLearner_;
...
class HookRegistrar {
public:
    HookRegistrar(Phase107TuningHooks& registry,
                 const std::string& hookName,
                 Phase107TuningHooks::HookCallback callback)
        : registry_(registry) {
        registry_.registerHook(hookName, callback);
    }

private:
    Phase107TuningHooks& registry_;
};
