...
#include "BankAwareCodeGenerator.hpp"
#include <sstream>
#include <algorithm>

namespace phase99 {

void BankAwareCodeGenerator::initializeBankLayout() {
    cacheManager_.startFunction();
    optimizer_.analyzeBankSetupPlacement();
}

std::map<std::string, FarVarCodeGenDecision> BankAwareCodeGenerator::analyzeFarVariableCodeGen() {
    decisions_.clear();
...
std::string BankAwareCodeGenerator::generateBankSetupCode(const std::string& varName) {
...
std::string BankAwareCodeGenerator::generateLoopOptimizedCode(const std::string& varName,
                                                            bool inLoop) {
    if (!inLoop) {
...
void BankAwareCodeGenerator::enterLoop(const std::string& loopLabel) {
    cacheManager_.enterLoop(loopLabel);
}

void BankAwareCodeGenerator::exitLoop() {
    cacheManager_.exitLoop();
}

std::string BankAwareCodeGenerator::getOptimizationHint(const std::string& varName) const {
...
void BankAwareCodeGenerator::recordVariableAccess(const std::string& varName,
                                                int accessCount,
                                                bool inLoop) {
...
std::string BankAwareCodeGenerator::generateBankImmediate(int bank) const {
    std::ostringstream code;
    code << "lda #$" << std::hex << bank << std::dec;
    return code.str();
}

std::string BankAwareCodeGenerator::generateBankStore() const {
    return "sta $FFF8";
}

bool BankAwareCodeGenerator::isCached(const std::string& varName) const {
    int bank = integrator_.getBankForVariable(varName);
