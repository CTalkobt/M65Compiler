...
void OptimizationCatalog::initializeCatalog() {
    ...
    catalog_.push_back({
        ...
        OptimizationCategory::BOTH, 1, -1, "-OConstantFolding", false, 5
    });

    catalog_.push_back({
        "DeadCodeElimination", "Dead Code Elimination",
        "Remove unreachable code and dead statements",
        OptimizationCategory::SPACE, 1, -1, "-ODeadCodeElimination", true, 3
    });
    ...
    catalog_.push_back({
        ...
        OptimizationCategory::SPEED, 2, -1, "-OInlineSmallFunctions", true, 2
    });

    catalog_.push_back({
        "TailCallOptimization", "Tail Call Optimization",
        "Convert tail calls to jumps (JSR + RTS → JMP)",
        OptimizationCategory::BOTH, 2, -1, "-OTailCallOptimization", true, 4
    });
    ...
    catalog_.push_back({
        ...
        OptimizationCategory::BOTH, 3, -1, "-OStrengthReduction", false, 3
    });

    catalog_.push_back({
        "AlgebraicSimplify", "Algebraic Simplification",
        "Eliminate identity/annihilator patterns (a*1=a, a*0=0)",
        OptimizationCategory::BOTH, 3, -1, "-OAlgebraicSimplify", false, 2
    });
    ...
    catalog_.push_back({
        ...
        OptimizationCategory::SPEED, 4, -1, "-OLoopUnrolling", true, 5
    });

    catalog_.push_back({
        "LoopInterchange", "Loop Interchange",
        "Reorder nested loops for better cache locality",
        OptimizationCategory::SPEED, 4, -1, "-OLoopInterchange", true, 4
    });

    catalog_.push_back({
        "LICM", "Loop-Invariant Code Motion",
        "Hoist loop-invariant computations outside loops",
        OptimizationCategory::SPEED, 4, -1, "-OLICM", true, 3
    });
    ...
    catalog_.push_back({
        ...
        OptimizationCategory::SPACE, 5, -1, "-OCrossFunctionInlining", true, 3
    });

    catalog_.push_back({
        "Devirtualization", "Devirtualization",
        "Replace single-implementation virtual calls with direct calls",
        OptimizationCategory::BOTH, 5, -1, "-ODevirtualization", true, 2
    });
    ...
    catalog_.push_back({
        ...
        OptimizationCategory::SPEED, 6, -1, "-OCSE", true, 4
    });

    catalog_.push_back({
        "CopyPropagation", "Copy Propagation",
        "Replace copies with original values to reduce moves",
        OptimizationCategory::BOTH, 6, -1, "-OCopyPropagation", true, 2
    });
...
std::string OptimizationCatalog::getDocumentation() {
    ...
    doc << "cc45 input.c -O3 -PNoLICM          # Level 3 but disable LICM\n";
    doc << "cc45 input.c -O0 -OConstantFolding # Start at 0, enable only ConstantFolding\n";
