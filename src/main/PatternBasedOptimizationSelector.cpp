...
#include "PatternBasedOptimizationSelector.hpp"
#include <algorithm>
#include <sstream>
#include <iomanip>
#include <numeric>
...
AdaptiveSelectionResult PatternBasedOptimizationSelector::selectHints(
...
SelectionStrategy PatternBasedOptimizationSelector::generateStrategy(
...
PatternBasedOptimizationSelector::filterRecommendations(
    const std::vector<OptimizationRecommendation>& recommendations,
    const SelectionStrategy& strategy) {
...
PatternBasedOptimizationSelector::resolveConflicts(
...
double PatternBasedOptimizationSelector::estimateCombinedBenefit(
    const std::vector<std::shared_ptr<phase100::OptimizationHint>>& hints,
    const CodePattern& pattern) {
...
std::string PatternBasedOptimizationSelector::generateSelectionReport(
...
double PatternBasedOptimizationSelector::getAverageConfidence() const {
...
bool PatternBasedOptimizationSelector::isConflicting(
...
double PatternBasedOptimizationSelector::calculateSynergyBonus(
    const std::vector<std::shared_ptr<phase100::OptimizationHint>>& hints) {
...
std::string PatternBasedOptimizationSelector::formatSelectionRow(
