...
void CodeGenerator::visit(BinaryOperation& node) {
    ...
    if (scale == 1 && !anyIs32 && (node.op == "+" || node.op == "-")) {
    ...
    if (isLiteralOne) {
        emitLiteralOneOptimization(node, scale, lhsType);
        return;
    }
    ...
    if ((node.op == "*" || node.op == "/" || node.op == "%" || node.op == "<<" || node.op == ">>") &&
        dynamic_cast<IntegerLiteral*>(node.right.get())) {
        emitMultiplicativeLiteral(node, lhsType);
        return;
    }
