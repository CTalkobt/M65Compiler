...
DependencyGraph DependencyTracker::buildDependencyGraph(
...
void DependencyTracker::analyzeDependencies(DependencyGraph& graph) {
...
std::vector<ModuleCluster> DependencyTracker::identifyModuleClusters(
...
double DependencyTracker::calculateCohesion(
...
DependencyTracker::findOptimizationBoundaries(
    const DependencyGraph& graph) {
...
std::string DependencyTracker::generateOptimizationStrategy(
...
std::string DependencyTracker::generateDependencyReport(
    const DependencyGraph& graph) {
...
double DependencyTracker::getAverageCohesion() const {
...
int DependencyTracker::countInternalDependencies(
    const ModuleCluster& cluster,
    const DependencyGraph& graph) {
    int count = 0;
...
int DependencyTracker::countExternalDependencies(
    const ModuleCluster& cluster,
    const DependencyGraph& graph) {
    int count = 0;
    std::set<std::string> members(cluster.members.begin(), cluster.members.end());
...
std::string DependencyTracker::formatEdgeRow(const DependencyEdge& edge) {
