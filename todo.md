...
- [ ] **PETSCII/ASCII encoding mismatch**: Character literals (`'A'`) use raw ASCII values ($41) but string literals (`"hello"`) go through the assembler's `.text` PETSCII conversion which swaps case (ASCII `'A'` $41 → PETSCII $61, ASCII `'a'` $61 → PETSCII $41). This means `*str == 'A'` fails when `str` points into a PETSCII-encoded string because the byte values differ. Affects any C code that compares characters from strings against char literals. The `sprintf`/`printf` implementations work around this by comparing against both cases (`*fmt == 'D' || *fmt == 'd'`). A proper fix would either: (a) apply the same PETSCII conversion to char literals in the C lexer, (b) add a `.ascii` directive that emits raw bytes without conversion for use by the compiler's string pool, or (c) make `.text` conversion configurable via a pragma. Option (b) is cleanest — it separates the encoding concern from the language.
...
- [ ] **`-O0` does not emit global pointer initializers**: With `-O0`, `volatile char *results = (char *)0x4000` emits `.res 2, 0` (uninitialized) instead of `.word $4000`. The ConstantFolder normally processes the initializer expression and folds it to a literal, but `-O0` skips constant folding entirely. The CodeGenerator's `emitData()` then sees a non-literal initializer and falls back to BSS reservation. Workaround: don't use `-O0` with initialized global pointers, or use `__asm__` to set the value.
