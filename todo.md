...
- [X] **O45Emitter: `@` local labels not relocated**: `jmp @mainloop` in `.o45` object files kept the object-local address in linked PRGs instead of being relocated. Root cause: the O45Emitter's relocation scanning loops didn't restore `currentLocalScope_` from each statement's `localLabelScope`, so `resolveSymbol("@mainloop", "")` failed to find the scoped symbol `_test:@mainloop`. Fixed by setting `parser.currentLocalScope_ = stmt->localLabelScope` at the top of each scanning loop. Reproducer in `bug/issue8/`.
- [X] **O45Emitter: symbol+offset expressions not relocated**: `sta table+4` generated no relocation entry because the operand `table+4` is a compound expression, not a bare symbol name. `resolveSymbol("table+4")` returned nullptr. Fixed by falling back to the token stream: extract the first IDENTIFIER token at `operandTokenIndex` and resolve that as the base symbol. The constant offset is already baked into the binary, so the linker's `existingVal - origBase + newBase` arithmetic preserves it. Reproducer in `bug/issue9/`.
- [X] **BinaryExpr reentrancy (assembler expr only)**: Fixed. Added `usesHardwareMath()` virtual method to `ExprAST` hierarchy to detect which sub-expressions write to hardware math registers. In `BinaryExpr::emit()` for `+`/`-`/`*`/`/`, guarded the operand-load sequence with `if (right->usesHardwareMath())` to use `push_ax()`/`pop_ax()` only when needed (nested hardware math operations). Simple right operands (constants, variables) incur zero overhead. Fixed GitHub issue #35.
...
- [X] **`sprintf` with `%ld` produces empty output**: Fixed. Resolved ZP register conflicts between library and compiler pool, and added hardware divider busy-wait loops to stabilize math operations. Corrected return preservation and stack-relative addressing.
- [X] **ConstantFolder loses `long` type during constant propagation**: Fixed. Updated `ConstantFolder` to track and propagate C types along with folded values. `IntegerLiteral` nodes now preserve the source type (e.g. `long`), ensuring correct stack-push sizing for variadic functions.
- [X] **ConstantFolder eliminates branches guarded by reassigned variables**: Fixed. Implemented constant state merging in `IfStatement`. Constants are now only propagated past a branch if they are consistent across both `then` and `else` paths. Reassignments within branches correctly invalidate pre-existing constant assumptions.
- [X] **Assembler simulated op size drift**: Resolved. Simulated opcodes now produce consistent sizes across passes. The root cause was often related to uninitialized scratch ZP registers in `M65Emitter` or incorrect address resolution for forward-referenced symbols. Fixed by initializing scratch registers relative to `zeroPageStart` and improving sizing logic.
- [X] **Zero Page Preservation**: Implemented. ZP state ($08-$FF) is now saved and restored at program boundaries to prevent OS state corruption. Includes `#pragma no_zp_save` for explicit control.
- [X] **Unused Variable Warnings**: Implemented in `cc45`. Correctly identifies unused local variables and parameters, with full integration into the optimization passes.
- [X] **Procedure Scoping Validation**: Implemented in `ca45`. Detects nested, unclosed, or orphan procedure blocks.
- [X] **8-bit Operand Overflow Warnings**: Implemented in `ca45`. Warns about 16-bit values in 8-bit addressing modes.
- [X] **Unused Global Symbol Warnings**: Implemented in `ln45`. Identifies dead code at the link stage.
- [X] **Assembler `@` local label scoping**: `@`-prefixed labels outside of `proc` blocks were stored unscoped in the symbol table. Duplicate `@` labels across different routines (e.g., `@loop:` in two separate functions) caused the second definition to overwrite the first. In pass2's convergence loop, the symbol value oscillated between two addresses on each iteration, causing an infinite hang. Fixed: `@` labels are now automatically scoped under the nearest preceding non-`@` label (e.g., `_myfunc:@loop`). Duplicate `@` labels in the same scope are detected and reported as errors. A max iteration guard (100 passes) prevents hangs from any remaining convergence issues.
...
- [X] **Assembler `.var` positional evaluation**: Fixed. `.var` ASSIGN directives are now re-evaluated in source order during pass 2. Variables are reset to initial values before pass 2, then re-evaluated positionally. Temporary bumps (`.var x = x + 2` then `.var x = x - 1`) now correctly affect only references after each assignment.
- [X] **PETSCII/ASCII encoding mismatch**: Fixed. The C lexer's `lexChar()` now applies the same PETSCII case-swap conversion as the assembler's `.text` directive. Character literals `'a'`-`'z'` map to $41-$5A and `'A'`-`'Z'` map to $61-$7A, matching the bytes emitted by `.text` for string literals. Non-alpha characters are unchanged. This means `*str == 'd'` now works correctly when `str` points into a `.text`-encoded string.
- [X] **O45 reader mishandles $FF escape bytes in relocation tables**: The `parseRelocTable` in `O45Reader.cpp` treated every byte as a `delta, typeSeg` pair. The .o65 spec uses `$FF` as an escape byte meaning "advance 254 bytes, then read next delta" — no type/seg byte follows. Large C-compiled modules with relocations spaced >254 bytes apart produce `$FF` escapes, causing the reader to desync and report "truncated import name". Fixed by skipping the type/seg read when delta is `$FF`.
- [X] **Linker relocation for global variable access**: Fixed. Simulated ops (ldax/stax etc.) that emit absolute address references to named symbols were not generating relocation entries. Added `M65Emitter::recordSymbolReloc()` tracking and O45Emitter processing, matching the existing `spBaseRelocs` pattern.
- [X] **O45 reader/writer corrupt files for large modules**: Fixed. Root cause was the O45 reader not handling `$FF` escape bytes in relocation delta-offset streams. Large modules with relocations >254 bytes apart produce `$FF` escapes that desynchronized the reader. Writer was correct all along.
- [X] **BSS duplicate symbol conflicts**: Fixed. Reworked BSS architecture: `__bss_start`/`__bss_end` are now linker-defined symbols injected by `ln45`. Individual compilation units no longer emit them. CRT module `crt_bssinit.o45` provides `_init_bss` using linker-provided boundaries.
- [X] **O45Emitter symbol segment misassignment (Issue #45-2)**: Fixed. The O45Emitter used `findSegmentForAddress()` to infer segment membership from address ranges, which failed at segment boundaries. Symbols at the start of `.data` or `.bss` were misclassified as TEXT, causing the linker to overlap code and data at the same address. Fix: use the authoritative `sym->segment` field (set at parse time) for all symbol exports and relocation targets. Removed `findSegmentForAddress()` entirely. Also fixed missing `currentSegment->name` in Symbol initializers for `.array`, `proc`, `__fill_dma_buf`, and `__move_dma_buf`. Added cross-segment address collision warning in the linker and 55 new segment emission tests.
- [X] **Constant propagation not invalidated by address-of**: Fixed. When `&x` is applied to a non-const variable, the variable is added to `addressEscapedVars` and removed from `knownConstants`. Address-escaped variables are excluded from future constant propagation. `const` variables are exempt — their address can be taken safely since they can't be legally modified through the pointer.
- [X] **`-O0` does not emit global pointer initializers**: Fixed. Added `tryEvalConstInt()` — a recursive evaluator in `emitData()` that extracts compile-time constant values from expression trees (CastExpression, UnaryOperation `-`/`~`, BinaryOperation `+`/`-`/`*`/`/`/`%`/`&`/`|`/`^`/`<<`/`>>`). Replaces the `dynamic_cast<IntegerLiteral*>` checks so initializers like `(char *)0x4000`, `-5`, and `0x100 + 0x20` emit correct `.word`/`.byte` values even without the ConstantFolder.
...
- [X] **Processor flag/register intrinsics**: `__cpu.A`, `__flags.Carry`, etc. High-level access with optimized emission (SEC/CLC, LDX/LDY). (Completed 2026-05-22).
...
- [ ] **AX Increment/Decrement**: Support inc <register> — Increment/decrement `.AX` as a 16-bit pair. Verify functionality doesn't already exist.  Register can be any of .A, .X, .Y, .Z, .AX, .AY, .XY, etc... 
...
- [ ] **`crt45.s` — C Runtime Startup**: Modular CRT archive (`crt45.lib`) linked into every C program. Infrastructure for `#pragma crt` feature flags is implemented. Individual CRT modules:
    ...
    - [X] **`#pragma crt no_bssinit`**: Skip BSS zeroing at startup. In flat mode, `_init_bss` is emitted inline. In linked mode, `crt_bssinit.o45` provides `_init_bss` using linker-defined `__bss_start`/`__bss_end` symbols.
...
- [X] **`string.h`**: `strlen`, `strcpy`, `strncpy`, `strcmp`, `strncmp`, `strcat`, `strncat`, `strchr`, `strrchr`, `strstr`, `strpbrk`, `strspn`, `strcspn`, `strtok`, `memcpy`, `memmove`, `memset`, `memcmp` (18 functions). Validated via mmemu emulator tests (`test_strlen`, `test_strcmp`, `test_strcpy`, `test_memcpy`, `test_strchr`).
- [X] **`stdio.h`**: `putchar`, `puts` (KERNAL CHROUT wrappers), `getchar` (KERNAL GETIN), `sprintf`, `printf` (variadic formatted output with `%d`, `%u`, `%x`, `%s`, `%c`, `%%`, `%o`, `%b`, `%p`). Long format support via `l` modifier: `%ld`, `%lu`, `%lx`, `%lo`, `%lb`. `sscanf` (formatted string input with `%d`, `%u`, `%x`, `%o`, `%c`, `%s`, `%%`, plus long variants `%ld`, `%lu`, `%lx`, `%lo`).
...
- [X] **`ctype.h`**: `isdigit`, `isalpha`, `isalnum`, `isspace`, `isprint`, `isupper`, `islower`, `isxdigit`, `ispunct`, `isblank`, `iscntrl`, `toupper`, `tolower` (13 PETSCII-aware functions). Validated via build + link.
- [X] **stdlib.h** (remaining): `abs`, `atoi`, `itoa`, `ltoa`, `strtol`, `strtoul`, `rand`, `srand`, `atos`, `stoa`. `rand` uses MEGA65 hardware RNG at `$D7EF` with busy-wait on `$D7FE` bit 7. `srand` is a no-op (hardware RNG). `atos`/`stoa` are macros casting through `atoi`/`itoa`. `ltoa` converts 32-bit long values using hardware divider. `strtol`/`strtoul` support base auto-detection, PETSCII letter ranges.
...
## Installation
- [ ] **Snap Install**: Add creation of a snap file for easy installation. (bin/ lib/ man/). 
