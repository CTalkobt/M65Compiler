...
- [I] **`sprintf` with `%ld` produces empty output**: `ltoa` works correctly when called directly from C, but produces empty output when called through the `emit_ltoa` helper inside `sprintf`. Suspected ZP conflict between compiler-allocated ZP temps ($02-$0A) and ltoa's hardcoded ZP usage. ltoa.s ZP relocated to $0B-$13 but issue persists. The `ptrstack` address for the tmp buffer may be incorrect in the nested call context, or indirect writes via `($0B),Y` to stack-relative addresses have a subtle timing issue. Direct ltoa calls to absolute addresses ($4000) work; calls with stack pointers ($01A6) through sprintf's emit_ltoa helper do not. Needs further tracing of the actual pointer value at the `STA ($0B),Y` instruction inside the nested call.
- [ ] **ConstantFolder loses `long` type during constant propagation**: When the ConstantFolder replaces a `long`-typed variable reference (e.g., `long a = 26L;` → uses of `a` become `IntegerLiteral(26)`), the resulting literal has no type annotation and `getExprType` returns `"int"`. This causes variadic argument pushes to use 2 bytes instead of 4, corrupting the `va_arg` read sequence. Affects `sprintf`/`printf` with `%ld` when the long value fits in 16 bits. Workaround: compile with `-O0`, or use global long variables (not constant-folded). Root fix: either annotate IntegerLiteral with a source type, or prevent constant propagation of `long` variables.
- [ ] **ConstantFolder eliminates branches guarded by reassigned variables**: The ConstantFolder propagates the initial value of a local variable (e.g., `int isLong = 0;`) through subsequent `if (isLong)` checks, eliminating branches as dead code — even when the variable is reassigned inside a conditional (`if (...) isLong = 1;`). The folder lacks an `IfStatement` visitor that would invalidate the constant when the variable is assigned in a branch. Workaround: restructure code to avoid using a flag variable (e.g., check the format character directly in each branch). Affected: `sprintf`, `printf`, `sscanf` long format specifier support.
