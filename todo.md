...
- [ ] **O45 writer produces corrupt files for large C-compiled modules**: `nm45` reports "truncated import name" when reading `.o45` files generated from large C source files (e.g. `sprintf.c` at ~4KB binary / ~1159 lines of assembly). The `.o45` file is written but contains malformed symbol table entries. Root cause likely in `O45Writer` or `O45Emitter`: either the relocation table or symbol table overflows a size field, or the delta-offset relocation encoder (`O45RelocEncoder`) produces an incorrect byte stream when there are many relocation entries (large C files generate hundreds of stack-relative relocations via `__sp_base`). The issue prevents C-compiled library modules from being linked via `ln45 -l`. Hand-written assembly `.o45` files (e.g. `itoa.s`, `strlen.s`) are unaffected — they have fewer relocations. Workaround: include C library functions directly in the program source rather than linking from `stdlib45.lib`, or write performance-critical stdlib modules in assembly.
- [ ] **Constant propagation not invalidated by address-of**: When a local variable has its address taken (`&x`) and passed to a function, the ConstantFolder still propagates the variable's pre-call value to subsequent reads. If the called function modifies the variable through the pointer, the propagated constant is stale. Workaround: declare the variable `volatile`. A proper fix would mark variables as "address-escaped" when `&` is applied and exclude them from constant propagation.
