; assert_fail.s — Runtime assertion failure handler (ZP calling convention)
;
; Called by assert() macro when an assertion fails.
; Prints an error message to the console and halts execution.
;
; When called from ZP-convention code, this function internally converts
; to stack convention for the printf call (since assert macro always
; pushes arguments to stack).
;
; Parameters: See comment above for how assert() passes arguments
