/* audio_midi.c — MEGA65 MIDI Input Handler & SID Router (Phase 11)
 *
 * MIDI message parsing and SID synthesizer control.
 * Converts MIDI messages to synthesis parameters.
 */
...
/* ============================================================================
 * MIDI-TO-SID VOICE ROUTER - Convert MIDI to SID synthesis
 * ============================================================================ */
...
void midi_to_sid_t__init(struct midi_to_sid *this, sid_synth_t *synth) {
    ...
    /* Initialize voice allocation (up to 12 voices) */
    for (int i = 0; i < 12; i++) {
...
void midi_to_sid_t__note_on(struct midi_to_sid *this, int channel, int note, int velocity) {
    ...
    /* Allocate voice (round-robin for omni mode) */
    int voice = (channel * 3 / 16) % this->num_voices;
    if (this->omni_mode) {
        /* Find first inactive voice across all available voices */
    ...
    /* Apply channel-specific parameters */
...
void midi_to_sid_t__note_off(struct midi_to_sid *this, int channel, int note) {
    ...
    /* Find and release the voice playing this note (search all voices) */
    for (int i = 0; i < this->num_voices; i++) {
        if (this->voice_alloc[i].active &&
...
void midi_to_sid_t__control_change(struct midi_to_sid *this, int channel, int cc, int value) {
    ...
    switch (cc) {
        ...
        case MIDI_CC_FILTER_CUTOFF:
            /* Set filter cutoff (0-20000 Hz) for all voices on channel */
            for (int i = 0; i < this->num_voices; i++) {
        ...
        case MIDI_CC_FILTER_RESONANCE:
            /* Set filter resonance (0-255) for all voices on channel */
            for (int i = 0; i < this->num_voices; i++) {
                if (this->voice_alloc[i].midi_channel == channel) {
...
void midi_to_sid_t__pitch_bend(struct midi_to_sid *this, int channel, int bend) {
     ...
     * Apply as frequency offset ±2 semitones */
    for (int i = 0; i < this->num_voices; i++) {
        if (this->voice_alloc[i].midi_channel == channel && this->voice_alloc[i].active) {
            int bend_cents = (bend * 200) / 8192;  /* ±2 semitones = ±200 cents */
...
void midi_to_sid_t__program_change(struct midi_to_sid *this, int channel, int program) {
    /* Placeholder: Could load preset instrument sounds */
    (void)this;
    (void)channel;
    (void)program;
}
...
void midi_to_sid_t__channel_pressure(struct midi_to_sid *this, int channel, int pressure) {
    /* Channel aftertouch: could modulate vibrato or filter */
    if (pressure > 0) {
...
void midi_receiver_t__handle_message(struct midi_receiver *this,
    ...
    switch (command) {
        ...
        case MIDI_PITCH_BEND:
            /* Combine two 7-bit values into 14-bit bend value */
...
int midi_note_to_frequency(int note) {
     ...
     * Simplified: use lookup table or interpolation
     * For now, approximate calculation */
    int semitones_from_a4 = note - 69;
    int freq = 440;

    /* Apply multiplier for each semitone */
    for (int i = 0; i < semitones_from_a4; i++) {
        freq = (freq * 1059) / 1000;  /* Approximate 2^(1/12) */
    }
...
int frequency_to_midi_note(int freq) {
    ...
    /* Reverse calculation: find closest MIDI note */
    int closest_note = 69;
