...
private:
    ...
    // Pattern matching: detect memcpy/memset/reduction/search/count/dotproduct opportunities
    ...
    bool isDotProductPattern(const ForStatement& stmt, std::string& accum, std::string& arrayA, std::string& arrayB, std::string& idx);
    ...
    std::unique_ptr<Statement> transformDotProductToCall(const ForStatement& stmt, const std::string& accum, const std::string& arrayA, const std::string& arrayB);
