...
| Global | `_g_` | Data segment | Absolute | `_g_total` |
...
Note the double prefix: the C variable `g_value` becomes `_g_g_value` in assembly (the compiler prepends `_g_` to the user-chosen name `g_value`). For a variable named `total`, the assembly name would be `_g_total`.
...
void set_flag(char f) {
    ...
    __asm__("sta _g_g_flag");   // 8-bit absolute store
}
```

#### Accessing Parameters (Stack-Relative)

Function parameters live on the hardware stack. The assembler's `proc` directive assigns each parameter a stack offset, and the `, s` suffix triggers stack-relative addressing (synthesized as `TSX; LDA/STA $0101+offset,X`).

For 16-bit (`int`, pointers) parameters, use `ldax`/`stax`:
```c
...
Local variables also live on the stack. The compiler tracks their offsets via `.var` directives. Volatile locals are always read from and written to the stack (non-volatile locals may be optimized away):

```c
int copy_value(int val) {
    volatile int result = 0;
    __asm__("ldax _p_val, s");       // Load parameter
    __asm__("stax _l_result, s");    // Store to local
...
```
        ...
        │   Caller's frame │
        ├──────────────────┤
        │   Parameter N    │  ← _p_paramN (pushed first, highest offset)
        │       ...        │
        │   Parameter 1    │  ← _p_param1
        ├──────────────────┤
        │  Return address  │  ← 2 bytes (pushed by JSR)
        ├──────────────────┤
        │   Local var 1    │  ← _l_var1 (offset 0, top of stack)
        │       ...        │
        │   Local var M    │  ← _l_varM
        └──────────────────┘  ← SP points here
```

Parameter offsets are set by the `proc` declaration and automatically adjusted by `.var` directives as locals are pushed. The offsets are managed by the assembler — inline assembly simply uses the symbolic names with `, s` and gets the correct offset.
...
- **Global Variables**: Prefixed with `_g_` (e.g., `total` becomes `_g_total`). Stored in the `.data` segment at absolute addresses.
