...
- `-fzpcall`: Use the zero-page parameter passing convention. Parameters are stored in a fixed ZP region instead of being pushed onto the stack. See [ZP Calling Convention](#zp-calling-convention) for details.
- `-fno-zpcall`: (Default) Use the traditional stack-based parameter passing convention.
...
When `-fzpcall` is enabled, function parameters are passed via a fixed zero-page region instead of being pushed onto the hardware stack. This eliminates TSX-based stack-relative addressing for parameter access, producing shorter and faster code.
...
- `long` params: 4 bytes (little-endian)

The caller stores arguments into the ZP block before `JSR`. The callee reads them directly with fast ZP addressing (`lda $03` instead of `TSX; LDA $0101+offset,X`).

### Example

```c
// Compiled with -fzpcall
int add(int a, int b) {
    return a + b;
}
// a is at ZP $03/$04, b is at ZP $05/$06
// No TSX needed — direct ZP load/store
```

### Caller-Save Semantics

When a function makes nested calls, its own ZP parameters must be saved to the stack frame before the call and restored afterward. Leaf functions (no calls) skip this entirely. The compiler detects leaf functions automatically via the `CallCollector` AST walker.

### Address-of Parameters

If `&param` is used, the parameter is spilled from ZP to a pre-allocated frame slot at function entry. All subsequent access goes through the frame slot, which has a stable runtime address. The ZP slot is freed.
...
| `struct` | Hidden pointer in ZP $03/$04 |

### Limitations

- Maximum parameter bytes = `zeroPageAvail - 1` (default: 8 bytes)
- Variadic functions always use stack-based passing (variable-length args)
- Indirect calls (function pointers) use stack-based passing (callee unknown at compile time)
- Not reentrant by default — interrupt handlers must save the ZP param region

### Function Metadata

In `-c` (relocatable) mode, each function emits metadata directives that are stored in the `.o45` symbol table:
- `.zp_uses` — ZP slots read as parameters
- `.zp_clobbers` — ZP slots written (caller must save if live)
- `.reg_clobbers` — CPU registers clobbered (A, X, Y, Z)

The linker reads this metadata to build call graphs and compute transitive clobber sets. See [zpcall.md](zpcall.md) for the full design document.

### Mixing Conventions

The default convention (`-fno-zpcall`) is fully preserved. All existing tests pass unchanged. A ZP-convention standard library (`stdlib45_zp.lib`) and CRT (`crt45_zp.lib`) are provided alongside the stack-convention versions.
...
- [zpcall.md](zpcall.md) — ZP calling convention design
